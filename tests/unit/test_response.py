...
def test_lpdaac_forward_handler(
    ...
    s3_forward_bucket: Bucket,
    s3_forward_trigger_object: Object,
) -> None:
    ...
    results = handler(
        sns_event_discrepancies, None, hls_forward_bucket=s3_forward_bucket.name
    )
...
def test_no_discrepancies(
    ...
    # subject contains "Ok"
    assert {} == handler(
        sns_event_no_discrepancies, None, hls_forward_bucket=s3_forward_bucket.name
    )
